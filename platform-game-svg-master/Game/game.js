...
function createGoodThing() {
	...
	while(i < 8){
		var a = Math.floor(Math.random() * 600);
		var b = Math.floor(Math.random() * 560);
		var hasCollision = false;
	    for (var j = 0; j < platforms.childNodes.length; j++) {
	        var node = platforms.childNodes.item(j);
	        if (node.nodeName != "rect") continue;
...
function monsterShoot(monsterNode) {
	...
	// 	monsterBullet--;
	    ...
	    if(player.position.x > monsterNode.getAttribute("x")) {
	    	bullet.setAttribute("x", parseFloat(monsterNode.getAttribute("x")) + MONSTER_SIZE.w/2  - BULLET_SIZE.w );
	    ...
	    bullet.setAttribute("y", parseFloat(monsterNode.getAttribute("y")) + MONSTER_SIZE.h / 2 - BULLET_SIZE.h / 2);
